...
await conn.sendMessage(from,{document: {url: data.datalist.list[0].file.path_alt},fileName: data.datalist.list[0].name,mimetype: 'application/vnd.android.package-archive',caption: `> *POWERED by ARROW-MD*`},{quoted: mek})
...
`
	 ...
	 const mass = await conn.sendMessage(from, { image: { url: `https://i.ibb.co/SR76mBh/Pu3-ZYHBS5139.jpg`  }, caption: `${pakaya}\n\n` }, { quoted: mek });
...
async(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        let message = {
            header: 'Queen_Ahinsa-MD SIHALASUB.LK MOVIE SEARCH',
            footer: '> *POWERED by Queen_Ahinsa-MD CONNECTED*',
...
`;
        ...
        conn.ev.on('messages.upsert', async (messageUpdate) => {
            ...
            if (isReplyToSentMsg) {
                ...
                await conn.sendMessage(from, { react: { text: '⬆️', key: mek.key } });
                if (messageType === '1') {
                    await conn.sendMessage(from, { document: { url: directLink }, mimetype: "video/mp4", fileName: `🎬 Queen_Ahinsa-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 480p\n\n> *POWERED by Queen_Ahinsa-MD*` }, { quoted: mek });
                } else if (messageType === '2') {
                    await conn.sendMessage(from, { document: { url: directLink1 }, mimetype: "video/mp4", fileName: `🎬 Queen_Ahinsa-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 720p\n\n> *POWERED by Queen_Ahinsa-MD*` }, { quoted: mek });
                } else if (messageType === '3') {
                    await conn.sendMessage(from, { document: { url: directLink2 }, mimetype: "video/mp4", fileName: `🎬 Queen_Ahinsa-MD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 1080p\n\n> *POWERED by Queen_Ahinsa-MD*` }, { quoted: mek });
                }
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        let message = {
            ...
            footer: '> *POWERED by Queen_Ahinsa-MD*',
            body: `*📌 Select Your Movie*`,
            image: `https://i.ibb.co/DLDD8Zy/73a9c4e2ea3c4cf1.jpg`
        };
...
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
▫️🔗 *Join :* https://whatsapp.com/channel/0029VatlhKF5vKAI4qzFSU3c
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        await conn.sendMessage(from, { 
            ...
            caption: `\n*${titel}*\n\n> *${si}*\n\n> *POWERED by Queen_Ahinsa-MD*`,
            mimetype: "video/mp4",
            fileName: `🎬 Queen_Ahinsa-MD 🎬${titel}.mp4`
        });
